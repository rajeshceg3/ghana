...
def verify_mobile_search_and_accessibility():
    with sync_playwright() as p:
        ...
        try:
            ...
            # Use first() or specific locator to avoid strict mode violation if desktop list is also present in DOM
            list_ul = page.locator("ul[aria-label='List of attractions']")

            # Find the visible one
            visible_ul = list_ul.first
            for i in range(list_ul.count()):
                if list_ul.nth(i).is_visible():
                    visible_ul = list_ul.nth(i)
                    break

            expect(visible_ul).to_be_visible()
